...
function runBlockYoutube() {
  const locales = {
    en: {
      ...
      success:
        "YouTube is now ad-free! Please note that you need to run this shortcut again if you reload the page.",
    },
    ru: {
      logo: "с&nbsp;AdGuard",
      alreadyExecuted: "Быстрая команда уже выполнена.",
      wrongDomain:
        "Эта быстрая команда предназначена для использования только на YouTube.",
      success:
        "Теперь YouTube без рекламы! Важно: при перезагрузке страницы вам нужно будет заново запустить команду.",
    },
    es: {
      logo: "con&nbsp;AdGuard",
      ...
      success:
        "¡YouTube está ahora libre de anuncios! Ten en cuenta que tienes que volver a ejecutar este atajo si recargas la página.",
    },
    de: {
      logo: "mit&nbsp;AdGuard",
      ...
      success:
        "YouTube ist jetzt werbefrei! Bitte beachten Sie, dass Sie diesen Kurzbefehl erneut ausführen müssen, wenn Sie die Seite neu laden.",
    },
    fr: {
      logo: "avec&nbsp;AdGuard",
      alreadyExecuted: "Le raccourci a déjà été exécuté.",
      wrongDomain:
        "Ce raccourci est censé d’être lancé uniquement sur YouTube.",
      success:
        "YouTube est maintenant libre de pub ! Veuillez noter qu’il faudra rééxecuter le raccourci si vous rechargez la page.",
    },
    it: {
      logo: "con&nbsp;AdGuard",
      alreadyExecuted: "Il comando è già stato eseguito.",
      wrongDomain:
        "Questa scorciatoia dovrebbe essere lanciata solo su YouTube.",
      success:
        "YouTube è ora libero da pubblicità! Si prega di notare che è necessario eseguire nuovamente questa scorciatoia se ricarichi la pagina.",
    },
    "zh-cn": {
      logo: "使用&nbsp;AdGuard",
      ...
      success:
        "现在您的 YouTube 没有广告！请注意，若您重新加载页面，您需要再次启动快捷指令。",
    },
    "zh-tw": {
      logo: "偕同&nbsp;AdGuard",
      ...
      success:
        "現在 YouTube 為無廣告的！請注意，若您重新載入該頁面，您需要再次執行此捷徑。",
    },
    ko: {
      logo: "AdGuard&nbsp;사용",
      ...
      success:
        "이제 광고없이 YouTube를 시청할 수 있습니다. 페이지를 새로고침 할 경우, 이 단축어를 다시 실행해야 합니다.",
    },
    ja: {
      logo: "AdGuard作動中",
      alreadyExecuted: "ショートカットは既に実行されています。",
      wrongDomain:
        "※このショートカットは、YouTubeでのみ適用されることを想定しています。",
      success:
        "YouTubeが広告なしになりました！※YouTubeページを再読み込みした場合は、このショートカットを再度実行する必要がありますのでご注意ください。",
    },
    uk: {
      logo: "з&nbsp;AdGuard",
      ...
      success:
        "Тепер YouTube без реклами! Проте після перезавантаження сторінки необхідно знову запустити цю швидку команду.",
    },
  };
...
.ytmusic-nav-bar#left-content #${LOGO_ID} { display: block; }`;
    ...
    const addAdGuardLogo = () => {
      ...
      if (window.location.hostname === "m.youtube.com") {
        const btn = document.querySelector(
          "header.mobile-topbar-header > button",
        );
        if (btn) {
  ...
  const scriptText = pageScript
    .toString()
    .replace("__logo_text__", getMessage("logo"));
